"""results/ablation/plot_results.py

读取 results/ablation 目录下的消融实验 NPZ 结果，生成精简的 3 个统计表格和 3 张学术对比图：

【表格 (CSV)】:
1. overall_summary.csv: 各 Benchmark 下每个消融变体的总汇总表 (Runs, IGD Mean/Std, HV Mean/Std, 可行解数, 耗时)
2. algorithm_scores.csv: 消融变体在不同 Benchmark 下的得分表 (Mean ± Std)
3. algorithm_rankings.csv: 消融变体在不同 Benchmark 下的比较排名与 Wilcoxon 显著性检验表 (+/=/-, 平均排名)

【图表 (PNG)】:
1. igd_score_chart.png: 消融变体在不同 Benchmark 下的 IGD 得分对比图 (各问题 Mean ± Std 分面柱状图)
2. hv_score_chart.png: 消融变体在不同 Benchmark 下的 HV 得分对比图 (各问题 Mean ± Std 分面柱状图)
3. overall_ranking.png: 消融变体跨 Benchmark 的总体平均排名对比图 (Friedman Rank 对比)

用法:
    python results/ablation/plot_results.py
"""
...
ALGORITHM_ORDER = ["DSOCOL", "DSOCOL1", "DSOCOL3", "DSOCOL4"]
ALGORITHM_LABELS = {
    "DSOCOL": "DSOCOL (Full)",
    "DSOCOL1": "w/o NGSS",
    "DSOCOL3": "w/o COL",
...
def compute_statistics_and_reports(
    ...
    for algo in algorithms:
        ...
        if algo == control_algo:
            wilcoxon_row[f"{label} IGD Rank"] = "Base (Full)"
            wilcoxon_row[f"{label} HV Rank"] = "Base (Full)"
        else:
...
def plot_metric_chart(
    ...
    """绘制消融变体在不同 Benchmark 下的分面柱状图 (IGD 或 HV)。"""
    ...
    fig.suptitle(
        f"Ablation Study Comparison: {metric} Scores across Benchmarks {direction_note}",
        fontsize=14,
        fontweight="bold",
        color="#0F172A",
        y=0.98,
    ...
    plt.close(fig)
    print(f"📊 已生成消融图表: {output_path.name}")
...
def plot_overall_ranking_chart(
    ...
    """绘制消融变体跨 Benchmark 的总体平均排名对比图 (1x2 双子图)。"""
    algorithms = plot_data["algorithms"]
    ...
    ax1.set_title(
        "Ablation: Average IGD Ranking across 6 Benchmarks\n(Lower Rank Number is Better ↓)",
        fontsize=11.5,
        fontweight="bold",
        color="#0F172A",
        pad=10,
    ...
    ax2.set_title(
        "Ablation: Average HV Ranking across 6 Benchmarks\n(Lower Rank Number is Better ↓)",
        fontsize=11.5,
        fontweight="bold",
        color="#0F172A",
        pad=10,
    ...
    fig.suptitle(
        "Ablation Study Ranking Comparison (DSOCOL vs Variants)",
        fontsize=13.5,
        fontweight="bold",
        color="#0F172A",
        y=0.98,
...
def main() -> None:
    ...
    print(f"🚀 开始处理消融实验结果目录: {results_dir}")

    raw_df = load_all_runs(results_dir, ALGORITHM_ORDER)
    if raw_df.empty:
        raise RuntimeError(f"在 {results_dir} 下未找到有效的消融变体 NPZ 实验数据！")
