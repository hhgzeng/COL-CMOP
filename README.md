...
├── algorithms/                 # [算法实现库]
│   ├── __init__.py
│   ├── dsocol/                 # DSOCOL (双群体协同正交学习) 算法包
│   │   ├── __init__.py
│   │   ├── formulas.py         # 论文公式文件: 汇集公式 (2)--(9) 的所有核心数学逻辑与算子
│   │   └── algorithms.py       # 论文算法流程文件: 汇集 Algorithm 1--5 的所有伪代码流程
│   └── apsea/                  # APSEA (自适应种群规模进化算法) 对比算法包
│       ├── __init__.py
│       ├── algorithm.py        # APSEA 主算法框架 (包含三类自适应分支)
│       ├── fitness.py          # SPEA2-like 适应度计算 (对应 CalFitness.m)
│       └── selection.py        # CDP、NoConstrained、Epsilon 三类环境选择与 Truncation 截断
│
├── problems/                   # [Benchmark 问题集 (Python/pymoo)]
│   ├── cdtlz.py                # 校准后的 C-DTLZ 问题集 (C1DTLZ1, C1DTLZ3, C2DTLZ2, C3DTLZ1, C3DTLZ4)
│   ├── dascmop.py              # DAS-CMOP 问题集
│   └── dcdtlz.py               # DC-DTLZ 问题集
│
...
│   ├── run_dsocol.py           # DSOCOL 单算法演示运行入口
│   └── run_apsea.py            # APSEA 单算法演示运行入口
│
├── results/                    # [实验结果持久化存储]
...
### 2. 运行单算法演示示例
- **DSOCOL 在 C1DTLZ1 上运行**：
  ```bash
  PYTHONPATH=. uv run python tests/run_dsocol.py --max-evals 30000 --seed 42
  ```
- **APSEA 在 C1DTLZ1 上运行**：
  ```bash
  PYTHONPATH=. uv run python tests/run_apsea.py --max-evals 30000 --seed 42
  ```
