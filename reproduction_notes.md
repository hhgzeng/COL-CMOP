...
1. **C-DTLZ 系列 (4 个问题)**：`C1DTLZ1`, `C1DTLZ3`, `C2DTLZ2`, `C3DTLZ4`
   - _特点_：在传统 DTLZ 标准多目标测试函数上引入线性和非线性约束，构建了复杂的几何约束边界。
2. **DC-DTLZ 系列 (6 个问题)**：`DC1DTLZ1`, `DC1DTLZ3`, `DC2DTLZ1`, `DC2DTLZ3`, `DC3DTLZ1`, `DC3DTLZ3`
   - _特点_：带有断裂、不可行阻隔和陷阱块的约束问题，主要用于检验算法穿越断裂不可行区域的能力。
3. **DAS-CMOP 系列 (9 个问题)**：`DASCMOP1` ~ `DASCMOP9`
   - _特点_：具有独立可调的可行性难度、收敛难度与多样性难度的可调约束多目标问题集。
4. **LIR-CMOP 系列 (14 个问题)**：`LIRCMOP1` ~ `LIRCMOP14`
   - _特点_：包含大面积不可行阻隔、极度狭窄可行通道及多局部 Pareto 陷阱的复杂约束测试集（由 PlatEMO 源码手动 Python 重构）。
...
│
├── results-exp/                # [5. 主对比实验统计数据与可视化图表]
│   ├── overall_summary.csv              # 各 Benchmark 下算法总汇总表
│   ├── algorithm_scores.csv             # 各 Benchmark 下算法得分表 (Mean ± Std)
...
└── results-ablation/           # [6. 消融实验统计数据与可视化图表]
    ├── overall_summary.csv              # 各 Benchmark 下消融变体总汇总表
    ├── algorithm_scores.csv             # 各 Benchmark 下消融变体得分表
    ├── algorithm_rankings.csv           # 各 Benchmark 下消融变体排名与 Wilcoxon 检验表
    ├── igd_score_chart.png              # 消融变体 IGD 得分对比图
    ├── hv_score_chart.png               # 消融变体 HV 得分对比图
    ├── overall_ranking.png              # 消融变体总体平均排名柱状图
    └── plot_results.py                  # 一键生成图表与表格脚本
```
...
- **IGD 跨问题分布子图**：
  ![各 Benchmark 问题 IGD 表现](results-exp/igd_score_chart.png)

- **HV 跨问题分布子图**：
  ![各 Benchmark 问题 HV 表现](results-exp/hv_score_chart.png)
...
#### 消融实验整体排名与各 Benchmark 表现

根据 6 个 Benchmark 测试问题的消融对比统计：

![消融实验总体排名柱状图](results-ablation/overall_ranking.png)

- **各消融变体 IGD 表现**：
  ![各 Benchmark 问题消融 IGD 表现](results-ablation/igd_score_chart.png)

- **各消融变体 HV 表现**：
  ![各 Benchmark 问题消融 HV 表现](results-ablation/hv_score_chart.png)

#### 核心机制贡献分析

1. **NGSS 模块的作用 (`DSOCOL1` vs `DSOCOL`)**：缺少 NGSS 模块的 `DSOCOL1` 在复杂不可行约束问题（如 `LIR-CMOP1`）上指标严重下滑（IGD 从 `0.1267` 劣变至 `0.3198`），解集易向局部可行块集聚；而配备 NGSS 机制的 `DSOCOL` 依靠三层生态位划分与拥挤度控制，能够将解均匀铺满整个 Pareto 前沿。
2. **COL 模块的作用 (`DSOCOL3` vs `DSOCOL`)**：`DSOCOL` 能够沿趋势方向的正交补空间进行无冗余采样，解集向真 Pareto 前沿靠拢的速度与覆盖质量显著优于切除 COL 的 `DSOCOL3`。
3. **Trend Learning 模块的作用 (`DSOCOL4` vs `DSOCOL`)**：缺少趋势学习的 `DSOCOL4` 依靠随机矢量方向寻找出口，在全局探索效率与收敛性上均落后于具备收敛方向引导的完整版 `DSOCOL`。

---

## 4. 复现总结

本次对论文 **_Collaborative Orthogonal Learning for Constrained Multi-Objective Optimization_** (IEEE TEVC 2026) 的复现取得了完整成功：
